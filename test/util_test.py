...
class tidy2xhtml_test(unittest.TestCase):

    def self_closing_tag_test(s):
        """Self-closing tags shouldn't be rendered as just <tag/>, except for <br/>.
        See bug # 10"""
        s.assertNotEqual(u.strip_body_tags(u.tidy2xhtml("<i></i>")), '<i/>')

    def empty_tag_test(s):
        """Self-closing tags shouldn't be rendered as just <tag/>, except for <br/>.
        See bug # 10"""
        s.assertEqual(u.strip_body_tags(u.tidy2xhtml("hey: <i> </i>")),
                    "<p>hey: <i> </i></p>")

    def odd_tag_test(s):
        "If the html has an odd tag (<section> in this case), don't puke or skip it"
        s.assertNotEqual(u.strip_body_tags(u.tidy2xhtml(bad_input[0])), 
                         """<div>
...
""")
    ...
    def comment_test(s):
        ...
        s.assertEqual(u.strip_body_tags(u.tidy2xhtml(bad_input[1])), 
                      "<p>What about comments? <!-- comments! --> Do we like them?</p>")

    def br_test_1(s):
        "Do we handle <br>?"
        a = u.strip_body_tags(u.tidy2xhtml("<br>"))
    ...
    def br_test_4(s):
        "Do we handle <br></br>?"
        a = u.strip_body_tags(u.tidy2xhtml("<br></br>"))
        s.assertTrue(a == "<br/>" or a == "<br>")


def main():
    print u.strip_body_tags(u.tidy2xhtml(bad_input[0]))
